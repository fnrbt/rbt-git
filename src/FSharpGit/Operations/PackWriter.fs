...
module PackWriter =
    ...
    /// Stream-unpack a received packfile directly off `counting` (the pack starts
    /// at the stream's current position; CountingStream tracks pack offsets). Each
    /// object is written to disk as it is read, so memory stays bounded regardless
    /// of pack size: ofs-delta bases are looked up by offset->hash and read back
    /// from disk rather than cached in memory. Returns the written object ids.
    let unpackPackStream (repo: Repo) (counting: PackData.CountingStream) : Result<GitHash[], string> =
        try
            ...
            if magic.Length < 4 then
                Ok [||] // no packfile (e.g. a delete-only push)
            elif not (magic.[0] = byte 'P' && magic.[1] = byte 'A' && magic.[2] = byte 'C' && magic.[3] = byte 'K') then
                Error "not a packfile"
            else
                PackData.readExact counting 4 |> ignore // version
                ...
                let writeObj (t: string) (content: byte[]) (entryStart: int) =
                    match ObjectWriter.writeRaw repo t content with
                    | Ok h -> byOffset.[entryStart] <- h; written.Add h
                    | Error e -> err <- Some e
                while i < count && err.IsNone do
                    ...
                    | 6 ->
                        ...
                        match byOffset.TryGetValue baseOffset with
                        | true, baseHash ->
                            match ReadObjects.readRawObject repo baseHash with
                            | Ok (bt, bc) -> writeObj bt (PackData.applyDelta bc delta) entryStart
                            | Error e -> err <- Some e
                        | _ -> err <- Some(sprintf "ofs-delta base not found at offset %d" baseOffset)
                    | 7 ->
                        let baseHash =
                    ...
                    | other -> err <- Some(sprintf "unsupported pack object type %d" other)
                    i <- i + 1
                // consume the 20-byte trailer if present
                (try PackData.readExact counting 20 |> ignore with _ -> ())
                match err with
                | Some e -> Error e
                | None -> Ok(written.ToArray())
